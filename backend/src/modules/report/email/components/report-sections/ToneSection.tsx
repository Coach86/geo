...
export const ToneSection: React.FC<ToneSectionProps> = ({ data }) => {
  ...
  const getSentimentColor = (status: string): string => {
    return status === 'green' ? '#0D47A1' : status === 'yellow' ? '#4527A0' : '#AD1457';
  };

  const getSentimentBgColor = (status: string): string => {
    return status === 'green' ? '#E3F2FD' : status === 'yellow' ? '#EDE7F6' : '#FCE4EC';
  };
      ...
      <Section className="p-6">
        ...
        <Row className="mb-10">
              ...
              <Column>
                <Row>
                  <Column className="w-full">
                    ...
                    >
                      <Text
                        className="text-3xl m-0 font-bold"
                        style={{ color: getSentimentColor(globalAvg.status) }}
                      >
                        {globalAvg.sentiment}
                      </Text>
                      <Text
                ...
                <Row>
                  <Column className="w-1/2 pr-2">
                    <Text className="text-sm font-medium text-gray-500 mb-1 m-0">
                      Most Positive
                    </Text>
                    <div
                      style={{
                  ...
                  <Column className="w-1/2 pl-2">
                    ...
                    >
                      <Text
                        ...
                        style={{ color: getSentimentColor(data.sentiments[2].status) }}
                      >
          ...
          <Column className="w-1/2 pl-4">
            ...
            <Section>
              {data.sentiments.map((sentiment: SentimentData, index: number) => (
                    ...
                    >
                      <Row>
                        <Column>
                          <Text
                            className="text-lg font-bold m-0"
                            style={{ color: getSentimentColor(sentiment.status) }}
                          >
                            {sentiment.sentiment}
                          </Text>
                        </Column>
                        <Column className="text-right">
                          <Text
                            ...
                            style={{ color: getSentimentColor(sentiment.status) }}
                          >
                            {sentiment.status === 'green'
                              ? 'Positive'
                              : sentiment.status === 'yellow'
                                ? 'Neutral'
                                : 'Negative'}
                          </Text>
                        </Column>
              ...
              <Column key={index} className="w-1/3 px-2">
                ...
                >
                  ...
                  <div style={{ marginBottom: '8px' }}>
                    ...
                    <Text className="text-sm m-0">{sentiment.positiveKeywords}</Text>
                  </div>

                  <div>
                    <Text className="text-xs font-medium text-gray-500 mb-1 m-0">Negatives:</Text>
                    <Text className="text-sm m-0">{sentiment.negativeKeywords}</Text>
                  </div>
                </div>
              </Column>
            ))}
