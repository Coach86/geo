...
export class AdminRawResponseController {
  ...
  async getRawResponsesByBatchExecution(@Param('batchExecutionId') batchExecutionId: string) {
    try {
      const responses = await this.rawResponseService.getRawResponsesByBatchExecution(batchExecutionId);
      if (responses.length === 0) {
        throw new NotFoundException(`No raw responses found for batch execution ID ${batchExecutionId}`);
      }
      return responses;
    } catch (error) {
  ...
  @ApiQuery({ name: 'batchExecutionId', required: false, description: 'Filter by batch execution ID' })
  @ApiQuery({ name: 'llmProvider', required: false, description: 'Filter by LLM provider' })
  @ApiQuery({
    name: 'promptType',
    required: false,
    description: 'Filter by prompt type (spontaneous, direct, comparison)',
  ...
  async getRawResponses(
    ...
    @Query('promptType') promptType?: 'spontaneous' | 'direct' | 'comparison',
  ) {
    const filters: any = {};

    if (batchExecutionId) filters.batchExecutionId = batchExecutionId;
    if (llmProvider) filters.llmProvider = llmProvider;
