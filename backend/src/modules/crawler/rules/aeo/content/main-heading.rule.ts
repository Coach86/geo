  ...
  async evaluate(url: string, content: PageContent): Promise<RuleResult> {
    ...
    if (h1Count === 1) {
      ...
      if (h1WordCount >= 3 && h1WordCount <= 10) {
        evidence.push(EvidenceHelper.success(`Exactly one H1 tag found - optimal length (${h1WordCount} words)`, { 
          code: primaryH1,
          target: '3-10 words',
          score: 60
        }));
        ...
        evidence.push(EvidenceHelper.warning(`Exactly one H1 tag found - too short (${h1WordCount} words)`, { 
          code: primaryH1,
        ...
        evidence.push(EvidenceHelper.warning(`Exactly one H1 tag found - too long (${h1WordCount} words)`, { 
          code: primaryH1,
          target: '3-10 words for +10 more points',
          score: 50
        }));
      ...
      evidence.push(EvidenceHelper.error('No H1 tag found', { 
      ...
      evidence.push(EvidenceHelper.error(`Multiple H1 tags found (${h1Count})`, { 
    ...
    if (h1Texts.length > 0) {
      ...
      if (primaryH1.length > 20) {
        evidence.push(EvidenceHelper.success('H1 appears descriptive', {
          score: 20
        }));
        score += 20;
        scoreBreakdown.push({ component: 'H1 appears descriptive', points: 20 });
      } else {
        evidence.push(EvidenceHelper.warning('H1 may lack descriptiveness', { 
      ...
      if (!isGeneric) {
        evidence.push(EvidenceHelper.success('H1 is not generic', {
          score: 10
        }));
        score += 10;
        scoreBreakdown.push({ component: 'H1 is not generic', points: 10 });
      } else {
        evidence.push(EvidenceHelper.error('H1 uses generic text', { 
      ...
      if (titleMatch) {
        ...
        if (commonWords.length > 0 && h1Lower !== titleLower) {
          evidence.push(EvidenceHelper.success('H1 relates to title tag but is unique', {
            score: 10
          }));
          score += 10;
          scoreBreakdown.push({ component: 'H1 relates to title but unique', points: 10 });
        } else if (h1Lower === titleLower) {
          evidence.push(EvidenceHelper.warning('H1 exactly matches title tag', { 
            target: 'Make H1 unique from title for +10 points',
            score: 0
          }));
          scoreBreakdown.push({ component: 'H1 exactly matches title', points: 0 });
        } else {
          evidence.push(EvidenceHelper.warning('H1 and title tag are completely different', { 
