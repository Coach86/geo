...
export class BatchExecutionController {
  ...
  @Get('projects-by-trigger')
  @ApiOperation({ summary: 'Get projects by trigger type' })
  @ApiQuery({ name: 'triggerSource', required: false, description: 'Trigger source (cron, manual, project_creation, total)' })
  @ApiQuery({ name: 'date', required: false, description: 'Specific date (ISO format)' })
  @ApiQuery({ name: 'startDate', required: false, description: 'Start date for range (ISO format)' })
  @ApiQuery({ name: 'endDate', required: false, description: 'End date for range (ISO format)' })
  @ApiResponse({
    status: 200,
    description: 'Return projects filtered by trigger type and date',
  })
  async getProjectsByTriggerType(
    ...
    try {
      ...
      return await this.batchExecutionService.getProjectsByTriggerType(
        triggerSource,
        date,
        start,
        end,
