...
export class RawResponseService {
  ...
  async storeRawResponse(
    ...
    try {
      this.logger.log(`Storing raw response for ${promptType} prompt #${promptIndex} in batch execution ${batchExecutionId}`);
      ...
      const existingResponse = await this.rawResponseModel.findOne(query).exec();
      ...
      if (existingResponse) {
        ...
        this.logger.log(`Updating existing raw response with analyzer data for ${promptType} prompt #${promptIndex} in batch execution ${batchExecutionId}`);
