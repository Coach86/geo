...
export class MistralAdapter implements LlmAdapter {
  ...
  private mistralAI: ChatMistralAI;
  name = 'Mistral';
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (this.isAvailable()) {
      this.mistralAI = new ChatMistralAI({
        apiKey: this.apiKey,
  ...
  async call(prompt: string, options?: LlmCallOptions): Promise<LlmResponse> {
    ...
    try {
      ...
      // Configure Mistral client with updated options
      ...
      return {
        ...
        tokenUsage: {
          ...
          total: usage.total_tokens || usage.input_tokens + usage.completion_tokens || 0,
        },
      };
    } catch (error) {
