...
const BatchesTab: React.FC<BatchesTabProps> = ({ projectId, onRunNewBatch, onRunSingleBatch }) => {
  ...
  useEffect(() => {
    const fetchBatchExecutions = async () => {
      try {
        setLoading(true);
        const data = await getBatchExecutions(projectId);
      ...
      {batchExecutions.length === 0 ? (
        ...
        >
          <Typography variant="body1" sx={{ mb: 2, color: alpha(theme.palette.text.primary, 0.7) }}>
            No batch analyses have been run for this project yet.
          </Typography>
