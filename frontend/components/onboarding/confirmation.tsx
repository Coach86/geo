...
export default function Confirmation() {
  ...
  useEffect(() => {
    const configs = [formData, ...savedConfigs.filter((config) => config.id !== formData.id)].filter(
  ...
  const displayedConfig =
    viewingConfig && allConfigs.length > currentConfigIndex ? allConfigs[currentConfigIndex] : formData
  ...
  const selectedVisibilityPrompts = displayedConfig.visibilityPrompts?.filter((p) => p.selected) || []
  const selectedVisibilityCount = selectedVisibilityPrompts.length

  const selectedPerceptionPrompts = displayedConfig.perceptionPrompts?.filter((p) => p.selected) || []
  const selectedPerceptionCount = selectedPerceptionPrompts.length

  const selectedCompetitors = displayedConfig.competitors?.filter((comp) => comp.selected) || []
  const selectedModels = displayedConfig.llmModels?.filter((model) => model.selected) || []
  const totalPrompts = selectedVisibilityCount + selectedPerceptionCount + selectedCompetitors.length
  ...
  const totalPromptsAllConfigs = allConfigs.reduce((sum, config) => {
    const visibilityCount = config.visibilityPrompts?.filter((p) => p.selected).length || 0
    const perceptionCount = config.perceptionPrompts?.filter((p) => p.selected).length || 0
    const competitorsCount = config.competitors?.filter((comp) => comp.selected).length || 0
  ...
  const toggleSection = (section: "visibility" | "perception" | "comparison" | "models" | "urls") => {
  ...
  const navigateConfig = (direction: "prev" | "next") => {
    ...
    } else if (direction === "next" && currentConfigIndex < allConfigs.length - 1) {
      setCurrentConfigIndex(currentConfigIndex + 1)
    }
  }
    ...
    if (selectedConfig) {
      ...
      switch (step) {
        ...
        case 2:
          ...
          break
        case 3:
          router.push("/onboarding/model-selection")
          break
        default:
          router.push("/onboarding")
      }
  ...
  return (
    <div className="py-8 animate-fade-in">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-3xl font-bold mb-2 text-mono-900">Ready to generate your Report?</h1>
      ...
      {allConfigs.length === 0 && (
        <Alert variant="warning" className="mb-6 border-amber-200 bg-amber-50">
          ...
          <AlertDescription className="text-amber-800">
            You haven't configured any websites yet. Please add at least one website to generate a report.
          </AlertDescription>
        </Alert>
      )}
        ...
        <div className="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-medium flex items-center">
              ...
              <Badge className="ml-2 bg-secondary-100 text-secondary-700">{allConfigs.length}</Badge>
            </h2>
            <Button variant="outline" className="border-dashed" onClick={handleAddNewUrl}>
            ...
            {allConfigs.map((config, index) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium whitespace-nowrap">{config.website || "No URL"}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
          <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-center justify-between">
            <div className="flex items-center">
              ...
              <h2 className="font-semibold text-mono-900">{displayedConfig.website || "Website Configuration"}</h2>
              {displayedConfig.isEditing && <Badge className="ml-2 bg-amber-100 text-amber-800">Editing</Badge>}
            </div>
            <Button
              variant="outline"
              size="sm"
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-gray-50 p-3 rounded-md border border-gray-200">
                    ...
                    <div className="text-xl font-semibold">{displayedConfig.markets?.length || 0}</div>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-md border border-gray-200">
                    <div className="text-xs text-gray-500 mb-1">Languages</div>
                    <div className="text-xl font-semibold">
                      {displayedConfig.markets?.reduce((sum, market) => sum + (market.languages?.length || 0), 0) || 0}
                    </div>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-md border border-gray-200">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="font-medium">{displayedConfig.brandName || "Not provided"}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-md bg-accent-100 flex items-center justify-center">
                    ...
                    <div>
                      ...
                      <div className="font-medium">{displayedConfig.industry || "Not provided"}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-md bg-accent-100 flex items-center justify-center">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-700 mb-2 flex items-center">
                    ...
                    Visibility Prompts
                    <Badge className="ml-2 bg-secondary-100 text-secondary-700">{selectedVisibilityCount}</Badge>
                  </h3>
                  {selectedVisibilityCount > 0 ? (
                    ...
                    <p className="text-sm text-gray-500 italic pl-6">No visibility prompts selected</p>
                  )}
                </div>
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-700 mb-2 flex items-center">
                    ...
                    Perception Prompts
                    <Badge className="ml-2 bg-secondary-100 text-secondary-700">{selectedPerceptionCount}</Badge>
                  </h3>
                  {selectedPerceptionCount > 0 ? (
                    <ul className="space-y-2 pl-6">
                      {selectedPerceptionPrompts.map((prompt, index) => (
                        <li key={index} className="text-sm flex items-start">
                          <span className="text-secondary-400 mr-2">•</span>
                          <span>{prompt.text.replace("[Brand]", displayedConfig.brandName || "Your brand")}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-sm text-gray-500 italic pl-6">No perception prompts selected</p>
                  )}
                </div>
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-700 mb-2 flex items-center">
                    ...
                    Comparison Prompts
                    <Badge className="ml-2 bg-accent-100 text-accent-700">{selectedCompetitors.length}</Badge>
                  </h3>
                    ...
                    <ul className="space-y-2 pl-6">
                      {selectedCompetitors.map((competitor, index) => (
                        <li key={index} className="text-sm flex items-start">
                          ...
                          <span>{`${displayedConfig.brandName || "Your brand"} vs ${competitor.name} — which is better?`}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-sm text-gray-500 italic pl-6">No competitors selected for comparison</p>
                  )}
                </div>
              </div>
            </TabsContent>
                      ...
                      <div key={index} className="bg-gray-50 p-3 rounded-md border border-gray-200">
                        <div className="flex items-center mb-2">
                          <div className="w-8 h-8 rounded-md bg-accent-100 flex items-center justify-center mr-3">
                            {model.icon === "sparkles" && <Check className="h-4 w-4 text-accent-600" />}
                            {model.icon === "brain" && <Check className="h-4 w-4 text-accent-600" />}
                            {model.icon === "zap" && <Check className="h-4 w-4 text-accent-600" />}
                            {model.icon === "bot" && <Check className="h-4 w-4 text-accent-600" />}
                          </div>
                          <div>
                            <div className="font-medium">{model.name}</div>
                            {model.recommended && (
                              <Badge className="bg-accent-100 text-accent-700 text-xs">Recommended</Badge>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                          <div>
                            <span className="text-gray-500">Provider:</span> {model.provider}
                          </div>
                          <div>
                            <span className="text-gray-500">Web Access:</span> {model.webAccessDetail}
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <TabsContent value="brand" className="p-4 focus:outline-none">
              <div className="space-y-6">
                  ...
                  <div className="pl-6 space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="font-medium">{displayedConfig.website || "Not provided"}</div>
                      </div>
                      <div>
                ...
                <div>
                  ...
                  <div className="pl-6">
                    {displayedConfig.markets && displayedConfig.markets.length > 0 ? (
                      <div className="space-y-3">
                          ...
                          <div key={index} className="bg-gray-50 p-3 rounded-md border border-gray-200">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {market.languages?.map((lang, langIndex) => (
                                <Badge key={langIndex} variant="outline" className="bg-white">
                                  {lang}
                                </Badge>
                              ))}
                            </div>
                ...
                <div>
                  ...
                  <div className="pl-6">
                    <div className="flex flex-wrap gap-2">
                      {displayedConfig.attributes && displayedConfig.attributes.length > 0 ? (
      ...
      {allConfigs.length > 0 && (
        <div className="mt-8 text-center">
          ...
          <p className="text-xs text-gray-500 mt-2">
            ...
            prompts using {totalUniqueModels} AI model{totalUniqueModels > 1 ? "s" : ""}
          </p>
