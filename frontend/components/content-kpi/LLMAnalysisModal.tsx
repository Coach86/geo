...
export function LLMAnalysisModal({ projectId, pageUrl, trigger }: LLMAnalysisModalProps) {
  ...
  const copyToClipboard = async (text: string, section: 'prompt' | 'response') => {
    try {
        ...
        {data && !loading && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant={data.analysisType === 'unified' ? 'default' : 'secondary'}>
                {data.analysisType === 'unified' ? 'AI Analysis' : 'Static Analysis'}
              </Badge>
              ...
              {data.tokensUsed && (
                <Badge variant="outline">
                  ...
                  {data.tokensUsed.input + data.tokensUsed.output} tokens
                </Badge>
              )}
            </div>
                    ...
                    <CardHeader className="pb-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => copyToClipboard(data.response || '', 'response')}
                        >
            ...
            {data.tokensUsed && (
              <div className="text-xs text-muted-foreground">
                Token usage: {data.tokensUsed.input} input + {data.tokensUsed.output} output = {data.tokensUsed.input + data.tokensUsed.output} total
              </div>
            )}
          </div>
        )}
