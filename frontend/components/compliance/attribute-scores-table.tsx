...
import type {
  ...
  ToolUseInfo,
  SourceCitation,
} from "@/types/compliance"; // Adjust path if needed
...
interface CellData {
  ...
  // For now, let's assume clicking a cell shows details for ONE specific DetailedComplianceResult
  ...
  primaryDetailedResult?: DetailedComplianceResult; // The specific result to show in drawer
...
export default function AttributeScoresByModelTable({
  ...
  const tableData = useMemo(() => {
    ...
    const scoresMap: Record<
      ...
      Record<
        string,
        { sum: number; count: number; results: DetailedComplianceResult[] }
      >
    > = {};
