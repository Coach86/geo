...
export default function AccordSection({ data }: AccordSectionProps) {
  ...
  const highCount = data.attributes.filter((attr) => attr.alignment === "✅" || attr.alignment === "High").length

  const mediumCount = data.attributes.filter((attr) => attr.alignment === "⚠️" || attr.alignment === "Medium").length

  const lowCount = data.attributes.filter((attr) => attr.alignment === "❌" || attr.alignment === "Low").length
  ...
  return (
    <div className="mb-16 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="px-6 py-5 border-b border-gray-100 bg-gray-50">
        <h2 className="text-2xl font-bold text-gray-900">Accord — Brand Compliance</h2>
        <p className="text-sm text-gray-600 mt-1">
          Models' responses alignement with provided brand book and expected brand attributes
        </p>
      </div>
      ...
      <div className="p-6">
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mb-6">
            <div>
              <div className="flex items-center mb-1">
                <h3 className="text-3xl font-bold text-gray-900">{data.score.value}</h3>
            ...
            <div className="text-right">
              <div className="text-sm font-medium text-gray-900">Attributes Tested</div>
              <div className="text-2xl font-bold text-gray-900">{data.attributes.length}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="md:col-span-3">
            <h4 className="text-lg font-semibold text-gray-800 mb-4">Attribute Alignment</h4>
                ...
                <tbody>
                  {sortedAttributes.map((attribute, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-4 py-3 border-b border-gray-200">{attribute.name}</td>
                      ...
                      <td className={`px-4 py-3 border-b border-gray-200 ${getAlignmentClass(attribute.alignment)}`}>
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getAlignmentBgClass(attribute.alignment)}`}
                        >
          ...
          <div>
            <h4 className="text-lg font-semibold text-gray-800 mb-4">Alignment Summary</h4>
            ...
            >
              <div className="text-sm font-semibold text-gray-600 mb-1">
                ...
                <a href="/glossary" className="ml-1 text-gray-400 hover:text-gray-500">
                  <svg
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center p-2 bg-[#FCE4EC] rounded">
                <span className="text-sm font-medium text-[#AD1457]">Low Alignment</span>
        ...
        <div className="border-t border-gray-100 pt-6">
          <h4 className="text-lg font-semibold text-gray-800 mb-4">What is Accord Score?</h4>
          <p className="text-gray-600 mb-4">
            Accord score measures how well AI models' descriptions of your brand align with your desired brand
            attributes and messaging. A higher score indicates better alignment with your brand identity.
          </p>
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3">
                ...
                <ul className="mt-1 text-sm text-blue-700 list-disc pl-5 space-y-1">
                  <li>
                    Accord Score (x/10) = (Highly-aligned + partly-aligned -medium- responses) / Total responses tested
                    (coming from all analysis and prompts: Pulse, Tone, Arena)
                  </li>
                  <li>Sentiment « heat » analyzed by Claude Sonnet 3.7</li>
                  <li>Paramètres : 300 prompts…</li>
                </ul>
              </div>
            </div>
          </div>
